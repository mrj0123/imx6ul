...
char * recv_once_packet(cmd_head_t * pHead,int fd,struct sockaddr_un * src_addr,socklen_t * addrlen)
{
    ...
    myPtf("fd=%d recv_once_packet start!src_addr=%s\n",fd,src_addr->sun_path);
    if (pHead == NULL)
    {
        myPtf("head error!\n");
        return NULL;
    ...
    //一次收取不大于MAXLINE的数据
    memset(recv_once_pak,0,sizeof(recv_once_pak));
    ret = unsock_server_recvfrom(fd, (struct sockaddr *)src_addr, addrlen, recv_once_pak, MAXLINE , 5000);
    myPtf("recv_once_packet unsock_server_recvfrom recv_once_pak=%s!\n",recv_once_pak);
    memcpy(pHead,recv_once_pak,sizeof(cmd_head_t));
    if (ret != int(pHead->length+sizeof(cmd_head_t))) {
        //包身接收失败，返回
        myPtf("fd=%d recv data error %d\n",fd, ret);
        //清除文件？？再接收一次？？
        sleep(1);
        myPtf("fd=%d,recv data again to clean file!\n",fd);
        ret = unsock_server_recvfrom(fd, (struct sockaddr *)src_addr, addrlen, recv_once_pak, MAXLINE , 100);
        myPtf("fd=%d,recv data again ret = %d!\n",fd,ret);
        return NULL;
    }
    //分配包身
...
int c_send_once_packet(int fd, int cmd, int tsp, u8 *buf, size_t n ,char ser_path[108])
{
    ...
    for(int i=0;i<int(sizeof(cmd_head_t)+n);i++)
    {
...
int s_send_once_packet(int fd, int cmd, int tsp, u8 *buf, size_t n ,struct sockaddr_un * dst_addr,socklen_t * addrlen)
{
    ...
    char send_once_pak[MAXLINE];
    myPtf("s_send_once_packet send packet start!");
    cmd_head_t Head;
    Head.cmd = cmd;
    Head.length = n;
    Head.tsp = tsp;
    ...
    myPtf("s_send_once_packet send once packet data:");
    for(int i=0;i<int(sizeof(cmd_head_t)+n);i++)
    {
        myPtf("%02X",*(send_once_pak+i));
    }
    ...
    ///////////////////////////////////////////////
    myPtf("s_send_once_packet send packet over!\n");
    return ret;
}
...
char * s_recv_packet(cmd_head_t * pHead,int fd,struct sockaddr_un * src_addr,socklen_t * addrlen)
{
    ...
    //拷贝包头
    char recv_once_pak[MAXLINE];
    myPtf("fd=%d s_recv_packet start!src_addr=%s\n",fd,src_addr->sun_path);
    if (pHead == NULL)
    {
        myPtf("s_recv_packet head error!\n");
        return NULL;
    }
    //一次收取不大于MAXLINE的数据
    memset(recv_once_pak,0,sizeof(recv_once_pak));
    ret = unsock_server_recvfrom(fd, (struct sockaddr *)src_addr, addrlen, recv_once_pak, MAXLINE , 5000);
    myPtf("s_recv_packet unsock_server_recvfrom recv_once_pak=%s!\n",recv_once_pak);
    memcpy(pHead,recv_once_pak,sizeof(cmd_head_t));
    /*
    if (ret != int(pHead->length+sizeof(cmd_head_t))) {
        //包身接收失败，返回
    ...
    if (ret != int(pHead->length+sizeof(cmd_head_t))) {
        //包身接收失败，返回
        myPtf("s_recv_packet fd=%d recv data error %d\n",fd, ret);
        return NULL;
    }
    if (pHead->length==0)
    {
        myPtf("s_recv_packet recv head but no data!\n");
        return NULL;
    }
    //分配包身
    char * recvbuf =(char *) malloc(pHead->length);
    ...
    myPtf("s_recv_packet recv data OK!\n");
    return recvbuf;
}
...
char * c_recv_packet(cmd_head_t * pHead,int fd,struct sockaddr_un * src_addr,socklen_t * addrlen)
{
    ...
    */
    return recv_once_packet(pHead,fd,src_addr,addrlen);
}
