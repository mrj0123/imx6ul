                                   ...
                                   int pack)                    //填充方式
{
    ...
    len++;
    myPtf("解密后dbase64后,len=%d ",len);
    myPtf("解密后数据:");
    for (i = 0; i <= len; i++) {
        myPtf("%02X", *(output + i));//多看一位
    }
    myPtf("\n");
    free(tmp);
    free(tmp1);
...
//解密时：对输入的密文进行反base64操作，然后用cbc 3des 进行解密，最后再进行dbase64操作，最后得到明文
int DES_dbs64_3dcbc_decrypt(const unsigned char *input,   //输入待解密数据（密文）
                                   unsigned char *output,       //输出解密后数据（明文）
                                   long length,                 //输入数据长度  填8，表示按8字节数据进行加密
                                   char *key1,                  //8字节秘钥1
                                   char *key2,                  //8字节秘钥2
                                   char *key3,                  //8字节秘钥3
                                   DES_cblock ivec,            //偏移量
...
//加密：对原始待加密数据进行base64操作，并进行cbc 3des 加密，然后再进行base64操作，最后得到结果
int DES_3cbc_bs64_encrypt(const unsigned char *input,      //输入待加密数据
                                   unsigned char *output,       //输出加密后数据
                                   long length,                 //输入数据长度
                                   char *key1,                  //8字节秘钥1
                                   char *key2,                  //8字节秘钥2
